  ...
  return (
    <div className={`${props.wrapperclasses || ''} w-full relative`}>
      ...
      {props.type === 'password' && (
        ...
        >
          {localType === 'password' ? (
            <BsFillEyeFill className="w-[1.8rem] h-[1.8rem] text-gray-100 cursor-pointer" onClick={() => setLocalType('text')} />
          ) : (
            <BsFillEyeSlashFill className="w-[1.8rem] h-[1.8rem] text-gray-100 cursor-pointer" onClick={() => setLocalType('password')} />
          )}
        </div>
      )}
    </div>
