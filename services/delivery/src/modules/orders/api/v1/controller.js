...
order.post('/', celebrate({ [Segments.BODY]: createOrderSchema }), tracedAsyncHandler(async function controllerCreateOrder(req, res) {
    const data = await traced(serviceCreateOrder)(req.body);
    return toSuccess({ res, data, message: 'Order created successfully!' })
}));

order.get('/', filterQuery, tracedAsyncHandler(async function controllerGetOrders(req, res) {
...
order.get('/:id', celebrate({ [Segments.PARAMS]: objectIdSchema() }), tracedAsyncHandler(async function controllerGetOrderById(req, res) {
