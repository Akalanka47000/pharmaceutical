...
import { defaultLimiter as rateLimiter, authorizer, errorHandler, responseInterceptor } from './middleware';
...
clusterize(
  () => {
    ...
    app.use(`/api`, authorizer, rateLimiter, routes);
    ...
    app.listen(config.PORT, config.HOST, () => {
      logger.info(`Orchestrator listening on ${config.HOST}:${config.PORT}`);
    });
  },
  { logger },
);